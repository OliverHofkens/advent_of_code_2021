...
def trace_probe(velocity: Vec2, target_x: Vec2, target_y: Vec2) -> bool:
    """
    Simulates the trajectory of the probe.
    Returns:
        True if the probe hit the target, otherwise False
    """
    pos = [0, 0]
...
def main():
    ...
    guess_x = range(0, target_x[1] + 1, 1 if target_x[0] > 0 else -1)
    guess_y = range(target_y[0], 300)

    hits = [trace_probe((x, y), target_x, target_y) for x in guess_x for y in guess_y]
